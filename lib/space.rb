...
class Space
  ...
  def self.create(space_name:, description:, price:, user_id:)
    ...
    result = connection.exec("INSERT INTO spaces (space_name, description, price,user_id) VALUES('#{space_name}', '#{description}', '#{price}','#{user_id}') RETURNING space_id, space_name, description, price,user_id,available;")
